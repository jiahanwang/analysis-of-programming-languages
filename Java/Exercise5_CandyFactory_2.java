...
public class Exercise5_CandyFactory_2{
	
	public static String read_file(String file_path) throws IOException{
		return new String(Files.readAllBytes(Paths.get(file_path, new String[]{})));
	}
	
	public static String filter_chars_and_normalize(String str_data){
		return str_data.replaceAll("[\\W_]+", " ").toLowerCase();
	}
	...
	// Declare an interface so we can call the function 
	public interface Interim{
		public ArrayList<String> load_stop_words(String stop_words_path);
	}
	
	public static Interim remove_stop_words(String[] word_array)throws IOException {
		// Only final variables can be retrieved by inner classes
		final ArrayList<String> word_list = new ArrayList<String>(Arrays.asList(word_array));
		// An anonymous class definition may be based on either a non-final class or on an interface
		// Using an anonymous class to implement the Interim interface 
		return new Interim() {
			public ArrayList<String> load_stop_words(String stop_words_path){
				// Implements the interface
				ArrayList<String> stop_words;
				try {
					stop_words = new ArrayList<String>(Arrays.asList((new String(Files.readAllBytes(Paths.get(stop_words_path, new String[]{})))).split("[,]")));
					for(char i = 'a'; i <= 'z'; i++)
	...
	public static void main(String[] args) {
		try{
			List<Map.Entry<String, Integer>> top_words = sort(frequencies((remove_stop_words(scan(filter_chars_and_normalize(read_file(args[0])))).load_stop_words(args[1]))));		
